...
// Usage:
//   node downloadReceipts.cjs serviceAccount.json bucket [user1,user2]
// With no user IDs, downloads every file under receipts/<uid>/.
...
async function main() {
  ...
  if (!saPath || !bucketName) {
    console.error("Usage: node downloadReceipts.cjs serviceAccount.json bucket [user1,user2]");
    process.exit(1);
  }
